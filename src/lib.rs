...
impl PrometheusMetrics {
    ...
    pub fn new_with_registry(
        ...
        let http_requests_total =
            IntCounterVec::new(http_requests_total_opts, &["endpoint", "method", "status", "host"])
                .unwrap();
        registry
    ...
    fn update_metrics(&self, path: &str, method: &Method, status: StatusCode, host: &str, clock: SystemTime) {
        ...
        if let Ok(elapsed) = clock.elapsed() {
            ...
            self.http_requests_duration_seconds
                .with_label_values(&[path, &method, &status, host])
                .observe(duration);
        }
    ...
    S: Service<Request = ServiceRequest, Response = ServiceResponse<B>, Error = Error>,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        // get host from headers x-origin-domain
        let host = req
            .headers()
            .get("x-origin-domain")
            .map(|h| h.to_str().unwrap_or("").to_string())
            .unwrap_or_else(|| req.headers().get("host").unwrap().to_str().unwrap().to_string());
...
impl<B> PinnedDrop for StreamLog<B> {
    fn drop(self: Pin<&mut Self>) {
        ...
        self.inner
            .update_metrics(&self.path, &self.method, self.status, &self.host, self.clock);
    }
}
